<div class="mt-8 pt-8 border-t border-slate-100 dark:border-zinc-800">
    ...
    @if (session()->has('error'))
        <div
            class="mb-4 p-3 text-xs font-bold text-center text-red-700 bg-red-50 rounded-xl dark:bg-red-900/30 dark:text-red-300 border border-red-100 dark:border-red-800">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-2 gap-3">
        <button type="button" wire:click="loginAs('admin@teste.com')" wire:loading.attr="disabled"
            ...
            <span class="text-xs font-black uppercase tracking-wider">{{ __('Geral') }}</span>
            <span class="text-[10px] text-red-500/80 dark:text-red-500 font-medium mt-0.5">{{ __('admin@teste.com')
                }}</span>
        </button>
        <button type="button" wire:click="loginAs('processos@teste.com')" wire:loading.attr="disabled"
            ...
            <span class="text-[10px] text-blue-500/80 dark:text-blue-500 font-medium mt-0.5">{{
                __('processos@teste.com') }}</span>
        </button>
        <button type="button" wire:click="loginAs('gr@teste.com')" wire:loading.attr="disabled"
            ...
            <span class="text-[10px] text-emerald-500/80 dark:text-emerald-500 font-medium mt-0.5">{{ __('gr@teste.com')
                }}</span>
        </button>
    </div>
</div>