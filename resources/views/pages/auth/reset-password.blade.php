            ...
            <form method="POST" action="{{ route('password.update') }}" class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ request('email') }}" required
                        ...
                        class="block w-full px-5 py-4 bg-slate-100 dark:bg-zinc-800 border-none rounded-2xl text-slate-500 dark:text-zinc-500 font-bold text-sm cursor-not-allowed">
                    @error('email')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest ml-1 mb-2">
                            {{ __('Confirmar') }}
                        </label>
                ...
                @error('password')
                    <p class="mt-0 text-xs font-bold text-red-600 dark:text-red-400 ml-1">{{ $message }}</p>
                @enderror
